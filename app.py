...
project_dir = Path(__file__).parent / "ai_data_analytics_platform" / "project"
...
with st.sidebar:
    ...
    if use_sample:
        ...
        if not sample_path.exists():
            sample_path = Path(__file__).parent / "ai_data_analytics_platform" / "project" / "data" / "sample_sales_data.csv"
...
with get_tab("dashboard"):
    ...
    if date_col and val_metric:
        with row1[0]:
            st.plotly_chart(
                ...
                use_container_width=True, key="root_dashboard_trend_line", config=plotly_config
            )
            charts_rendered += 1
        ...
        with row1[0]:
            st.plotly_chart(
                ...
                use_container_width=True, key="root_dashboard_bar_sec1_left", config=plotly_config
            )
            charts_rendered += 1
    ...
    if primary_cat and val_metric:
        with row1[1]:
            st.plotly_chart(
                ce.category_bar_chart(filtered_df, primary_cat, val_metric, title=f"📊 Bar Chart: {val_metric} by {primary_cat}"),
                use_container_width=True, key="root_dashboard_category_bar", config=plotly_config
            )
            charts_rendered += 1
        ...
        with row1[1]:
            st.plotly_chart(
                ...
                use_container_width=True, key="root_dashboard_scatter_sec1", config=plotly_config
            )
            charts_rendered += 1
    ...
    if cat_or_reg and val_for_pie:
        with row2[0]:
            pie_type = st.radio("Chart Type:", ["Pie Chart 🥧", "Donut Chart 🍩"], horizontal=True, key="root_dash_pie_toggle")
            if "Pie Chart" in pie_type:
                st.plotly_chart(
                    ce.pie_chart(pie_df, cat_or_reg, val_for_pie, title=f"🥧 Pie Chart: {val_for_pie} Share by {cat_or_reg}"),
                    use_container_width=True, key="root_dashboard_pie_chart", config=plotly_config
                )
            else:
                st.plotly_chart(
                    ce.donut_chart(pie_df, cat_or_reg, val_for_pie, title=f"🍩 Donut Chart: {val_for_pie} Share by {cat_or_reg}"),
                    use_container_width=True, key="root_dashboard_donut_chart", config=plotly_config
                )
            charts_rendered += 1
        ...
        if geo_col_for_map and val_for_pie:
            try:
                st.plotly_chart(
                    ...
                    use_container_width=True, key="root_dashboard_map_chart", config=plotly_config
                )
                charts_rendered += 1
                map_rendered = True
            except Exception:
        ...
        if not map_rendered:
            if secondary_cat and val_for_pie and secondary_cat != primary_cat:
                st.plotly_chart(
                    ...
                    use_container_width=True, key="root_dashboard_sec_cat_bar", config=plotly_config
                )
                charts_rendered += 1
            elif len(numeric_cols) >= 2:
                st.plotly_chart(
                    ce.scatter_chart(filtered_df, numeric_cols[0], numeric_cols[1], title=f"🔵 Scatter Plot: {numeric_cols[1]} vs {numeric_cols[0]}"),
                    use_container_width=True, key="root_dashboard_scatter_r2", config=plotly_config
                )
                charts_rendered += 1
                ...
                st.plotly_chart(
                    ...
                    use_container_width=True, key="root_dashboard_donut_alt", config=plotly_config
                )
                charts_rendered += 1
    ...
    if revenue_col and profit_col and primary_cat:
        with row3[0]:
            st.plotly_chart(
                ...
                use_container_width=True, key="root_dashboard_grouped_bar", config=plotly_config
            )
            charts_rendered += 1
    elif len(numeric_cols) >= 2:
        with row3[0]:
            st.plotly_chart(
                ce.scatter_chart(filtered_df, numeric_cols[0], numeric_cols[1], title=f"🔵 Scatter Plot: {numeric_cols[1]} vs {numeric_cols[0]}"),
                use_container_width=True, key="root_dashboard_scatter_r3", config=plotly_config
            )
            charts_rendered += 1
    ...
    if primary_cat and val_metric:
        with row3[1]:
            st.plotly_chart(
                ...
                use_container_width=True, key="root_dashboard_box_chart", config=plotly_config
            )
            charts_rendered += 1
        ...
        with row3[1]:
            st.plotly_chart(
                ...
                use_container_width=True, key="root_dashboard_hist", config=plotly_config
            )
            charts_rendered += 1
...
with get_tab("explore"):
    ...
    if ctype == "numeric":
        st.plotly_chart(px.histogram(filtered_df, x=explore_col, nbins=30, template="plotly_white"),
                         use_container_width=True, key="root_explore_numeric_hist")
        ...
        st.plotly_chart(px.bar(x=counts.values, y=counts.index.astype(str), orientation="h",
                         ...
                         use_container_width=True, key="root_explore_cat_bar")
...
if "trends" in tab_keys:
    with get_tab("trends"):
        ...
        st.plotly_chart(ce.trend_line_chart(filtered_df, date_col, value_col), use_container_width=True, key="root_trends_trend_line")
        ...
        if forecast_df is not None:
            st.plotly_chart(ce.multi_series_trend(forecast_df, date_col, value_col), use_container_width=True, key="root_trends_forecast_multi")
    ...
    with get_tab("customer"):
        ...
        if value_col:
            st.plotly_chart(
                ...
                use_container_width=True, key="root_customer_top15_bar"
            )
        ...
        if roles.get("customer") and date_col and (revenue_col or profit_col):
            ...
            if not rfm.empty:
                st.plotly_chart(px.histogram(rfm, x="Segment", color="Segment", template="plotly_white",
                                              title="Customer Segments (RFM)"), use_container_width=True, key="root_customer_rfm_hist")
        ...
        if value_col:
            ...
            with col1:
                st.plotly_chart(ce.category_bar_chart(filtered_df, product_col, value_col, top_n=10,
                                 ...
                                 use_container_width=True, key="root_product_top10_bar")
            with col2:
                ...
                st.plotly_chart(px.bar(bottom, x=value_col, y=product_col, orientation="h",
                                 ...
                                 use_container_width=True, key="root_product_bottom10_bar")
    ...
    with get_tab("regional"):
        ...
        if value_col:
            st.plotly_chart(ce.category_bar_chart(filtered_df, region_col, value_col,
                             ...
                             use_container_width=True, key="root_regional_bar_chart")
            ...
            try:
                if map_col:
                    st.plotly_chart(ce.geo_choropleth(filtered_df, map_col, value_col),
                                     use_container_width=True, key="root_regional_geo_choropleth")
            except Exception:
            ...
            if profit_col and revenue_col:
                ...
                st.plotly_chart(px.bar(comp, x=region_col, y=["Revenue", "Profit"], barmode="group",
                                        template="plotly_white"), use_container_width=True, key="root_regional_rev_vs_prof_bar")
...
if "stats" in tab_keys:
    with get_tab("stats"):
        ...
        if len(numeric_cols) >= 2:
            ...
            st.plotly_chart(ce.correlation_heatmap(corr), use_container_width=True, key="root_stats_corr_heatmap")
